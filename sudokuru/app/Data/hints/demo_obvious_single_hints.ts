...
export type ObviousSingleDemoCase = {
  id: "single-obvious-single" | "obvious-single-with-note-simplification";
  label: string;
  puzzle: CellProps[][];
...
const SIMPLIFYING_PUZZLE_SOLUTION_NUMBERS: SudokuValue[][] = [
  [3, 1, 6, 9, 8, 4, 7, 5, 2],
  [2, 9, 8, 1, 5, 7, 3, 4, 6],
  [5, 7, 4, 6, 2, 3, 8, 1, 9],
  [4, 2, 3, 7, 1, 8, 6, 9, 5],
...
function getObviousSingleGroupHighlights(
  ...
  return [
    ...getObviousSingleGroupCells(target, group)
      .filter((location) => location.r !== target.r || location.c !== target.c)
      .map((location) => ({
        location,
...
export const obviousSinglePuzzle: CellProps[][] =
  numbersToPuzzleWithCandidateNotes(PLACEMENT_ONLY_SOURCE_PUZZLE_NUMBERS);

export const simplifyingObviousSinglePuzzle: CellProps[][] =
  numbersToPuzzleWithCandidateNotes(SIMPLIFYING_SOURCE_PUZZLE_NUMBERS);

export const obviousSinglePuzzleSolution: SudokuValue[][] =
  PLACEMENT_ONLY_SOURCE_PUZZLE_NUMBERS.map((row) =>
    row.map((value) => value || obviousSingleValueCell.value),
  );

export const simplifyingObviousSinglePuzzleSolution: SudokuValue[][] =
  SIMPLIFYING_PUZZLE_SOLUTION_NUMBERS;
...
export const obviousSingleDemoCases: ObviousSingleDemoCase[] = [
  ...
  },
  {
    ...
    solution: simplifyingObviousSinglePuzzleSolution,
