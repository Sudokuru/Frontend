...
// todo write test
test.describe.skip("Simplify Notes", () => {
  ...
  test("should simplify notes after correct value insertion when enabled", async ({
    ...
    const headerComponent = new HeaderComponent(resumeDrillGame);
    await headerComponent.profile.click();
    const profilePage = new ProfilePage(resumeDrillGame);
    ...
    await headerComponent.drawer.click();
    await headerComponent.drawerPlay.click();
    const playPage = new PlayPage(resumeDrillGame);
...
test.describe("undo", () => {
  ...
  test("Undo button should replace value erased on previous move from erase button", async ({
    ...
    const sudokuBoard = new SudokuBoardComponent(resumeDrillGame);
    await sudokuBoard.cell[8][0].click();
    await sudokuBoard.numPad[8].click();
    await sudokuBoard.erase.click();
    await sudokuBoard.cellHasValue(8, 0, "0");
    await sudokuBoard.undo.click();
    await sudokuBoard.cellHasValue(8, 0, "9");
  });
  ...
  // todo
  test.skip("Undo button should function with an erase move spanning multiple cells", async ({
    resumeDrillGame,
  }) => {
    const sudokuBoard = new SudokuBoardComponent(resumeDrillGame);
    ...
    await resumeDrillGame.keyboard.up("Shift");
...
// todo this test
test.describe.skip("numpad with multiple cells selected", () => {
  test.use({ classicGametoResume: NEW_EMPTY_GAME });
  test("inserting notes should succeed", async ({ resumeDrillGame }) => {
    const sudokuBoard = new SudokuBoardComponent(resumeDrillGame);
    await sudokuBoard.cell[3][6].click();
